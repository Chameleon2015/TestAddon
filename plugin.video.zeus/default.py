...
ZeusStreamDate = "30/10/2015 16:00hrs GMT"
...
print 'Zeus Addon ZeusAddonStartup#' + ZeusVideoBuild + '-' + str(osstat) + '  ' + str(ZeusStreamVersion) + '  ' + ZeusStreamDate
CompileData = 'NO DATA'
try: 
...
if CompileData == 'NO DATA': 
    xbmcgui.Dialog().ok('Zeus Video Server', 'This unit is not receiving data from the Zeus server', 'Test your connection speed & browse to zeusrepo.com','Seek support if this continues')
    print 'Zeus Addon no Compile Data * check connection to zeusrepo.com on this unit *'
else:
...
def ResolveIT():
    ...
    print 'InfoLabel ZeusAddon >>' + InfoLabel + '<<>>' + InfoLabel[-4:] + '<<Len>>' + str(len(InfoLabel)) + '<<'
    if InfoLabel[-2:] == 'US' or len(InfoLabel) < 10:
       return 0
    else:
       return 1
...
def UpdateMe():
    ...
    xbmcgui.Dialog().ok('Zeus Video Updated', 'A reboot may be required. ', '', 'Visit http://zeusrepo.com')
